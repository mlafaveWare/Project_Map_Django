...
select_org = sorted(df["Organization"].unique())
select_org_dropdown = st.sidebar.multiselect('Select one or more org(s) to display project data:', select_org)
selected_org_year = df[(df.Organization.isin(select_org_dropdown)) & ((df.Year <= startyear) & (df.Year >= endyear))]
view_state = pdk.ViewState(longitude=-100.10, latitude=35, zoom=2.5, min_zoom=1.5, max_zoom=17, bearing=0, pitch=0)
view_offices = df_offices[(df_offices.Office.isin(select_org_dropdown))]
...
textlay = pdk.Layer(type="TextLayer", data=df[(df.Organization.isin(select_org_dropdown)) & ((df.Year <= startyear) & (df.Year >= endyear))], pickable=True, get_position=["longitude", "latitude"], get_text="name", get_size=300, sizeUnits='meters', get_color=[0, 0, 0], get_angle=0, getTextAnchor= '"middle"', get_alignment_baseline='"bottom"')
...
with col1:
    st.subheader('Projects: Organization & Year Selected') 
    st.write('Displaying ', str(selected_org_year.shape[0]) + ' of ', str(df.shape[0]) + ' Projects On Map & Chart Below')
    st.pydeck_chart(pp)
            
with col2:
    st.subheader('Insights: Organization & Year Selected')
    ...
    if rb1 == 'Horizontal':
        fig1 = px.histogram(selected_org_year, y='Year', color_discrete_sequence=['rgb(224,68,3)'], text_auto=True)
        ...
        fig1 = px.histogram(selected_org_year, x='Year', color_discrete_sequence=['rgb(224,68,3)'], text_auto=True)
        fig1.update_layout(bargap=0.3)
        fig1.update_layout(barmode='relative')
        fig1.update_layout(plot_bgcolor='rgba(0,0,0,0)')
...
with col3:
    st.write('🟠 markers denote project locations by organization & 🔵 markers denote Ware Malcomb office location(s).')
    gb = GridOptionsBuilder.from_dataframe(selected_org_year.reset_index(drop=True))
    gb.configure_pagination(paginationPageSize=20)
    gridOptions = gb.build()
    AgGrid(selected_org_year.reset_index(drop=True), gridOptions=gridOptions)

with col4:
    rb = st.radio(label = 'View Data By:', options = ['By Org','By Office','By Project Type'], index=0)
    st.write('<style>div.row-widget.stRadio > div{flex-direction:row;}</style>', unsafe_allow_html=True)
    if rb == 'By Org':
        fig2 = px.sunburst(data_frame=selected_org_year, path=["Organization", 'Office', 'Project_Type'], color="Organization", color_discrete_sequence=['rgb(224,68,3)'],)
        ...
        fig2 = px.sunburst(data_frame=selected_org_year, path=["Office", 'Organization', 'Project_Type'], color="Office", color_discrete_sequence=['rgb(224,68,3)'],)
        fig2.update_layout(xaxis={'visible': False, 'showticklabels': False})
        fig2.update_layout(yaxis={'visible': False, 'showticklabels': False})
        fig2.update_layout(plot_bgcolor='rgba(0,0,0,0)')
    elif rb == 'By Project Type':
        fig2 = px.sunburst(data_frame=selected_org_year, path=["Project_Type", 'Organization', 'Office'], color="Project_Type", color_discrete_sequence=px.colors.sequential.RdBu)
        fig2.update_layout(xaxis={'visible': False, 'showticklabels': False})
        fig2.update_layout(yaxis={'visible': False, 'showticklabels': False})
        fig2.update_layout(plot_bgcolor='rgba(0,0,0,0)')
